...
class SegmentDataset:
    ...
    def build(self):
        for id in range(len(self.text)):
            token = self.tokenizer.encode_plus(self.text[id], add_special_tokens=True,pad_to_max_length=True, max_length=config.MAX_LENGTH, truncation='post')
            self.input.append(token['input_ids'])
            self.mask.append(token['attention_mask'])

    def __getitem__(self,idx):
        return (torch.tensor(self.input[idx], dtype=torch.long),
