...
def run():
    ...
    for num_fold, (df_train, df_valid) in enumerate(kfold_df(df)):
        ...
        num_train_steps = int(
                len_train_dataset / config.TRAIN_BATCH_SIZE*config.NUM_EPOCHS
            )
        ...
        es = utils.EarlyStopping(patience=2, mode="max")
        train_fn(model, train_dataset,optimizer, scheduler, len_train_dataset = len(train_dataset), device)
        model = model.load_state_dict(torch.load(config.MODEL_PATH))
        print(eval_fn(model,valid_loader,device))
